...
export function buildConfig({
  ...
  sharedSecret = process.env.CANVA_APP_SHARED_SECRET,
  ...
  return {
        ...
        },
        {
          ...
          use: [
            ...
            },
            {
              ...
              options: {
                postcssOptions: {
                  // This app lives nested inside the CanvaCodeCraft repo,
                  // which has its own postcss.config.js (for Tailwind) one
                  // directory up. postcss-loader's config search would
                  // otherwise walk up and pick that one up instead of the
                  // plugins list below, so config discovery is disabled.
                  config: false,
    ...
    plugins: [
      new DefinePlugin({
        ...
        CANVA_APP_SHARED_SECRET: JSON.stringify(sharedSecret ?? ""),
      }),
